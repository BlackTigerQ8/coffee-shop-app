...
const ResourceForm = ({ resource, categories, onSubmit, onCancel }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.unit) newErrors.unit = t("unit_required");
    if (!formData.currentStock && formData.currentStock !== 0)
      newErrors.currentStock = t("current_stock_required");
    if (!formData.minimumStock && formData.minimumStock !== 0)
      newErrors.minimumStock = t("minimum_stock_required");
    if (!formData.costPerUnit)
      newErrors.costPerUnit = t("cost_per_unit_required");
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (validateForm()) {
      onSubmit(formData, resource?._id);
    }
  };
...
const RestockForm = ({ resource, onSubmit, onCancel }) => {
  const { t } = useTranslation();
  ...
  return (
    <form onSubmit={handleSubmit}>
      <Typography variant="body1" sx={{ mb: 2 }}>
        {t("current_stock")}: {resource?.currentStock} {resource?.unit}
      </Typography>
      ...
      <TextField
        ...
        InputProps={{
          ...
          endAdornment: (
            <InputAdornment position="end">{resource?.unit}</InputAdornment>
          ),
        }}
      />
...
const ResourcesTable = React.forwardRef(({ categories, setIsLoading }, ref) => {
  const { t } = useTranslation();
  ...
  const renderRow = (resource) => {
    ...
    return (
      ...
      >
        ...
        <td className="p-4 text-dark">{resource.costPerUnit.toFixed(3)} KWD</td>
    ...
    <>
      ...
      <Stack spacing={2} sx={{ mt: 3, alignItems: "center" }}>
        <Typography sx={{ color: "#DA9F5B" }}>
          ...
          {Math.min(page * itemsPerPage, filteredResources.length)} {t("of")}{" "}
          {filteredResources.length} {t("resources")}
      ...
      >
        <DialogTitle>
          {currentResource ? t("edit_resource") : t("add_resource")}
        </DialogTitle>
        <DialogContent>
          <ResourceForm
      ...
      >
        <DialogTitle>
          {t("restock_resource")}: {currentResource?.name}
        </DialogTitle>
        <DialogContent>
          <RestockForm
            resource={currentResource}
